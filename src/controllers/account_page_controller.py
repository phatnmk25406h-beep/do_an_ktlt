...
def get_rounded_pixmap(image_path, width, height, radius):
    """Tạo QPixmap được bo tròn góc"""
    if not os.path.exists(image_path):
        return QPixmap() # Trả về pixmap rỗng nếu không tìm thấy ảnh
    ...
    # Tạo ảnh đầu ra với kênh alpha (trong suốt)
    rounded_pixmap = QPixmap(scaled_pixmap.size())
    rounded_pixmap.fill(Qt.GlobalColor.transparent)
    
    # Bắt đầu vẽ
    painter = QPainter(rounded_pixmap)
    painter.setRenderHint(QPainter.RenderHint.Antialiasing) # Bật khử răng cưa cho mượt
    painter.setRenderHint(QPainter.RenderHint.SmoothPixmapTransform)
    
    # Tạo mask (mặt nạ) hình chữ nhật bo góc
    path = QtGui.QPainterPath()
    path.addRoundedRect(QtCore.QRectF(scaled_pixmap.rect()), radius, radius)
    painter.setClipPath(path) # Chỉ cho phép vẽ bên trong mask
    
    # Vẽ ảnh đã scale vào mask
...
class AccountWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # --- NẠP VÀ BO TRÒN ẢNH KHỦNG LONG ---
        # Lấy thư mục gốc project để tạo đường dẫn động
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        img_path = os.path.join(base_dir, "trang_tri", "images", "motivation.jpg") # Đổi tên ảnh ở đây nếu bạn muốn
        
        # Bo tròn ảnh (Kích thước 100x100, góc bo 16px)
        rounded_dino = get_rounded_pixmap(img_path, 100, 100, 16)
        if not rounded_dino.isNull():
            self.ui.label_26.setPixmap(rounded_dino)
        else:
            print("Lỗi: Không tìm thấy ảnh tại", img_path)
        # -------------------------------------
        ...
        if user_data:
            self._minutes = user_data["minutes"]
            self.set_user_info(username=user_data["username"], level_text=f"Level {user_data['level']}")
        else:
            self._minutes = [0.0] * 7 # Nếu chưa học ngày nào thì mảng toàn số 0
        # ----------------------------------
    ...
    def _bar(self, ax):
        # Lấy giá trị cao nhất để vẽ màu đậm, nếu tất cả là 0 thì gán max = 1 để tránh lỗi chia 0
        max_v = max(self._minutes) if self._minutes and max(self._minutes) > 0 else 1
        
        colors = [_G_DARK if m == max_v and m > 0 else _G_LIGHT for m in self._minutes]
        ...
        # Tạo khoảng trống phía trên biểu đồ để số không bị cắt mất
        ax.set_ylim(0, max_v * 1.3)

        for b, v in zip(bars, self._minutes):
            if v > 0: # Chỉ hiển thị số lên đỉnh cột nếu số phút > 0
                # Làm tròn 1 chữ số thập phân (VD: 0.36 -> 0.4)
                ax.text(b.get_x() + b.get_width()/2, b.get_height() + (max_v * 0.05),
