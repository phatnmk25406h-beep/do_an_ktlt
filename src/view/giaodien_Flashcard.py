...
from .Flashcard_ui_mainwindow import Ui_MainWindow
...
def _load_cards_from_excel(topic_name=None) -> list:
    danh_sach_tu = []
    try:
        # 1. Lấy thư mục chứa file hiện tại (src/view)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # 2. Nhảy ra thư mục gốc (Gốc dự án - nơi chứa folder data và src)
        # Đi ngược ra 2 lần: từ 'view' -> 'src' -> 'gốc'
        project_root = os.path.dirname(os.path.dirname(current_dir))
        
        # 3. Kết nối tới file trong folder data
        excel_path = os.path.join(project_root, "data", "Flashcard.xlsx")
        
        print(f"Đang tìm file tại: {excel_path}") # Debug để bạn kiểm tra đường dẫn
        
        df = pd.read_excel(excel_path)
        danh_sach_tu = df.to_dict('records')
        # --- ĐOẠN CODE LỌC THEO CHỦ ĐỀ THÊM VÀO Ở ĐÂY ---
        if topic_name:
            # Chỉ lấy những từ mà giá trị cột 'topic' bằng với topic_name
            danh_sach_tu = [tu for tu in danh_sach_tu if tu.get('topic') == topic_name]
            
            # (Tùy chọn) In ra terminal để dễ debug xem lọc đúng không
            print(f"Đã tải {len(danh_sach_tu)} từ vựng cho chủ đề: {topic_name}")
    except Exception as e:
