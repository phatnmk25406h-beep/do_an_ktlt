...
# Import giao diện tĩnh theo đúng cú pháp bạn yêu cầu
from giaodien_baitap_sapxep.giaodien_sapxep import LessonUI


# ==========================================
# CLASS ĐỌC DỮ LIỆU TỪ EXCEL
# ==========================================
class LessonManager:
    ...
    def get_question(self, topic):
        ...
        all_words = []
        for d in self.data:
            all_words.extend(d['Đoạn tiếng anh'].split())
...
# ==========================================
# CLASS XỬ LÝ LOGIC UI VÀ SỰ KIỆN NÚT BẤM
# ==========================================
class LessonApp(LessonUI):
    def __init__(self):
        ...
        # 2. Khởi tạo Manager
        excel_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sapxeptu.xlsx")
        self.manager = LessonManager(excel_path)

        # 3. KẾT NỐI TẤT CẢ SỰ KIỆN (SIGNALS) Ở ĐÂY
    ...
    def load_new_question(self):
        data = self.manager.get_question("Travel and culture")
        if not data:
    ...
    def check_answer(self):
        ...
        if user_answer.strip() == correct_answer:
            ...
            self.continue_btn.setStyleSheet(
                "background-color: #58A700; border-radius: 25px; color: white; font-weight: bold;")
        else:
            ...
            self.continue_btn.setStyleSheet(
                "background-color: #EA2B2B; border-radius: 25px; color: white; font-weight: bold;")
