...
QMainWindow, QWidget {{ background: {GL}; color: {TXT}; }}
QScrollArea  {{ background: {GL}; border: none; }}
...
QLabel#nav_logo  {{ color: white; font-size: 15px; font-weight: bold; }}
QLabel#nav_sub   {{ color: rgba(255,255,255,0.85); font-size: 11px; }}
QPushButton#btn_nav_create {{
    background: {WH}; color: {G1}; border: 2px solid {WH};
    border-radius: 8px; font-weight: bold; padding: 7px 20px;
}}
QPushButton#btn_nav_saved {{
    background: transparent; color: {WH}; border: 2px solid {WH};
    border-radius: 8px; font-weight: bold; padding: 7px 20px;
}}
QPushButton#btn_nav_create:hover {{ background: {GL}; color:{G2}; }}
QPushButton#btn_nav_saved:hover  {{ background: rgba(255,255,255,0.15); }}
...
QPushButton#btn_add_card {{
    ...
    border: 2px dashed {G1}; border-radius: 10px;
    font-size: 15px; font-weight: bold; padding: 14px;
}}
QPushButton#btn_add_card:hover {{ background: {GL}; }}
...
class Ui_MainWindow(object):
    def setupUi(self, w):
        ...
        self.nav_bar.setFixedHeight(68)
        nl = H(self.nav_bar); nl.setContentsMargins(20, 0, 20, 0); nl.setSpacing(10)

        # logo square
        logo_sq = QtWidgets.QFrame()
        logo_sq.setFixedSize(40, 40)
        logo_sq.setStyleSheet("background:#388E3C; border-radius:10px;")
        logo_inner = V(logo_sq)
        logo_inner.setContentsMargins(0,0,0,0)
        lbl_icon = QtWidgets.QLabel("📖", parent=logo_sq)
        lbl_icon.setAlignment(C)
        lbl_icon.setStyleSheet("color:white; font-size:20px; background:transparent;")
        logo_inner.addWidget(lbl_icon)

        nl.addWidget(logo_sq)
        ...
        self.btn_save_set = btn(self.form_card, "btn_save_set", "💾  Save Set", 44, 140)
        ...
        hero.setStyleSheet("background:white; border-radius:0px; border-bottom:1px solid #E0E0E0;")
        hero.setMinimumHeight(160)
        hero_vl = V(hero); hero_vl.setAlignment(C)
        ...
        tips_frame.setStyleSheet("background:white; border-radius:12px; border:1px solid #E0E0E0;")
        tips_h = H(tips_frame); tips_h.setContentsMargins(24,20,24,20); tips_h.setSpacing(0)
        lbl_tips_h = lbl(tips_frame, "", "Study Tips", C)
        lbl_tips_h.setStyleSheet("font-size:18px; font-weight:bold;")
        tips_h.addWidget(lbl_tips_h)
        self.saved_vl.addWidget(tips_frame)
        self.saved_vl.addItem(SPV())
